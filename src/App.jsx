...
// Config
const BRAND = "Steve’s IT Pro";
const DOMAIN = "https://www.stevesitpro.com";
const CONTACT_EMAIL = "steve@stevesitpro.com";
const BUSINESS_PHONE = "TBD";
const CALENDLY_URL = "#";
const INTAKE_FORM_URL = "";
const INTAKE_SHEET_URL = "";
...
function getHeroSecondaryCta(url) {
  if (url && url.trim() !== "") return { href: url, label: "Fill discovery form", external: true };
  return { href: "#contact", label: "Contact us", external: false };
}
// Dev sanity checks
(function(){
  try {
    const a = getHeroSecondaryCta("https://forms.gle/x");
    console.assert(a.label === "Fill discovery form" && a.external === true);
    const b = getHeroSecondaryCta("");
    console.assert(b.href === "#contact" && b.label === "Contact us");
  } catch(_) {}
})();
...
const services = [
  { title: "Google Workspace Setup & Hardening", bullets: [
    "Tenant setup/migration guidance","Secure groups, OUs, & sharing controls",
    "SPF, DKIM, DMARC configuration","Drive/Shared Drives structure & cleanup",
  ]},
  { title: "Apps Script Automation & Integrations", bullets: [
    "Onboarding/Offboarding flows (Forms → Sheets → Admin)",
    "Doc/Drive automations & scheduled reports","3rd-party APIs & webhooks",
    "Custom admin tools & dashboards",
  ]},
  { title: "Admin & End-User Training", bullets: [
    "Google Workspace Admin fundamentals","Security & compliance best practices",
...
const tiers = [
  { name: "Starter Setup", price: "$749", cadence: "one-time", cta: "Get Started", popular: false, points: [
    "New or cleanup Workspace setup","SPF/DKIM/DMARC + basic policies",
    "Shared Drives structure blueprint","2 hours of admin coaching",
  ]},
  { name: "Automation Sprint", price: "$1,499", cadence: "per sprint (1–2 weeks)", cta: "Build My Workflow", popular: true, points: [
    "One scoped Apps Script workflow","Intake form + Sheet + approvals",
    "Email/slack notifications","Implementation & handoff video",
  ]},
  { name: "Pro Support", price: "$799", cadence: "/month", cta: "Book a Call", popular: false, points: [
    "Up to 8 hrs/mo consulting & fixes","Policy reviews & admin changes",
    "Quarterly security tune-ups","Priority response (next-business-day)",
  ]},
];

const testimonials = [
  { quote: "We went from manual onboarding chaos to a 1-click workflow. Saved us ~10 hours a week.", name: "Alicia P.", role: "Operations Lead, CraftSupply Co." },
...
export default function Site() {
  const orgSchema = { "@context":"https://schema.org","@type":"Organization",
    name: BRAND, url: DOMAIN, email: CONTACT_EMAIL, telephone: BUSINESS_PHONE, sameAs: [], logo: `${DOMAIN}/logo.png` };
  const serviceSchema = { "@context":"https://schema.org","@type":"Service",
    serviceType: "Google Workspace Consulting & Apps Script Development",
    provider: { "@type": "Organization", name: BRAND, url: DOMAIN },
    areaServed: "US",
    hasOfferCatalog: { "@type":"OfferCatalog", name:"Consulting Packages",
      itemListElement: tiers.map(t=>({ "@type":"Offer", name:t.name, price: t.price.replace(/[$,]/g,''), priceCurrency:"USD", url: DOMAIN + "#pricing" }))
    }
  };
  ...
  const submitDirect = async () => {
    if (form.website && form.website.trim() !== "") { setStatus({ type:"ok", note:"Thanks!" }); return; }
    if (!WEB_APP_URL) { setStatus({ type:"error", note:"Form not connected yet." }); return; }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-white to-slate-50 text-slate-900">
      ...
      <header className="sticky top-0 z-20 backdrop-blur bg-white/80 border-b border-slate-200">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <img src="/logo.svg" alt="Logo" className="h-9 w-9 rounded-2xl bg-blue-600 p-1" />
          ...
          <a href={calendlyCTA.href} target={calendlyCTA.external ? "_blank" : undefined} rel={calendlyCTA.external ? "noreferrer" : undefined} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white text-sm">
      ...
      <section className="mx-auto max-w-7xl px-4 pt-14 pb-10 grid md:grid-cols-2 gap-10 items-center">
        <div>
          <h1 className="text-4xl md:text-5xl font-bold leading-tight text-blue-700">Google Workspace Consulting & Apps Script Automation</h1>
          <p className="mt-4 text-slate-700 text-lg">Set up securely, automate the boring stuff, and train your team to fly. Side-hustle speed. Enterprise discipline.</p>
          <div className="mt-6 flex flex-col sm:flex-row gap-3">
            <a href="#pricing" className="px-5 py-3 rounded-2xl bg-green-600 text-white inline-flex items-center gap-2">See pricing <ArrowRight className="w-4 h-4"/></a>
            <a href={heroCTA.href} target={heroCTA.external ? "_blank" : undefined} rel={heroCTA.external ? "noreferrer" : undefined} className="px-5 py-3 rounded-2xl border border-yellow-500 text-yellow-600 inline-flex items-center gap-2">
              {heroCTA.label}
            </a>
          </div>
        ...
        <div className="relative">
          <div className="rounded-3xl bg-white shadow-lg border border-slate-200 p-5">
            ...
            {INTAKE_SHEET_URL ? (
              <a href={INTAKE_SHEET_URL} target="_blank" rel="noreferrer" className="mt-4 inline-block text-xs text-blue-600 underline">(See example intake sheet)</a>
            ) : null}
          </div>
        </div>
      </section>
          ...
          {services.map((s, i) => (
            <div key={i} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <ul className="mt-3 space-y-2 text-sm text-slate-700">
                {s.bullets.map((b, j) => (<li key={j} className="flex gap-2"><Check className="w-4 h-4 mt-0.5 text-yellow-500"/> {b}</li>))}
              </ul>
            </div>
          ))}
        </div>
      ...
      <section id="pricing" className="mx-auto max-w-7xl px-4 py-10">
        <div className="flex items-end justify-between gap-6 flex-wrap">
          ...
          <p className="text-sm text-slate-600 max-w-xl">Transparent pricing—edge over managed IT rates, premium over generic freelancers. Ask for fixed-fee if preferred.</p>
        </div>
        <div className="mt-6 grid md:grid-cols-3 gap-6">
          {tiers.map((t, i) => {
            const cta = resolveLink(INTAKE_FORM_URL);
            return (
              <div key={i} className={`rounded-3xl border ${t.popular ? "border-blue-600" : "border-slate-200"} bg-white p-6 shadow-sm relative`}>
                {t.popular && (<div className="absolute -top-3 right-6 text-xs px-2 py-1 bg-red-500 text-white rounded-full">Most Popular</div>)}
                ...
                <ul className="mt-3 space-y-2 text-sm text-slate-700">
                  {t.points.map((p, j) => (<li key={j} className="flex gap-2"><Check className="w-4 h-4 mt-0.5 text-yellow-500"/> {p}</li>))}
                </ul>
                <a href={cta.href} target={cta.external ? "_blank" : undefined} rel={cta.external ? "noreferrer" : undefined} className={`mt-5 inline-flex items-center gap-2 w-full justify-center px-4 py-2 rounded-xl ${t.popular ? "bg-green-600 text-white" : "border border-blue-600 text-blue-600"}`}>
                  {t.cta} <ArrowRight className="w-4 h-4"/>
                </a>
                <div className="mt-3 text-xs text-slate-500">Need custom? Hourly consulting: <span className="font-medium">$95/hr</span>. Advanced scripting/security: <span className="font-medium">$125/hr</span>.</div>
              </div>
            );
          })}
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-10">
        <div className="rounded-3xl bg-blue-600 text-white p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <div>
            ...
            <p className="text-white/80 text-sm mt-1">Free 30-min discovery • 48-hour prototype on most automations • 14-day warranty & fixes • Clear documentation & handoff video</p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <a href={calendlyCTA.href} target={calendlyCTA.external ? "_blank" : undefined} rel={calendlyCTA.external ? "noreferrer" : undefined} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-yellow-400 text-slate-900">
              Book a free consult <ArrowRight className="w-4 h-4"/>
            </a>
            <a href={resolveLink(INTAKE_FORM_URL).href} target={resolveLink(INTAKE_FORM_URL).external ? "_blank" : undefined} rel={resolveLink(INTAKE_FORM_URL).external ? "noreferrer" : undefined} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white text-blue-600 border border-white">
      ...
      <section id="contact" className="mx-auto max-w-7xl px-4 pb-16">
        <div className="grid md:grid-cols-2 gap-8">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-600">Tell us about your Workspace environment and what you’d like to automate or improve. We’ll reply within one business day.</p>
            <form className="mt-4 grid gap-3" onSubmit={(e)=>{e.preventDefault(); submitDirect();}}>
              ...
              <textarea className="border border-slate-300 rounded-xl px-3 py-2" placeholder="What problem should we solve?" rows={5} value={form.message} onChange={e=>setForm({...form, message:e.target.value})}/>
              {/* Honeypot */}
